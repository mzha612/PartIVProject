...
Diameter_Data = readtable('cap_dia_gamma .csv');
Diameter_Data = table2array(Diameter_Data);
%% TODO manual with coefficients

% Length_data = load('Frequency_Length.mat');

Length_data = readtable('cap_len_gamma .csv');
...
%         Vessel3{c}.Radius = Sample_Diameter("Capillary",Diameter_Data)/2;
        Vessel3{c}.Radius = Diameter_Data(randi(size(Diameter_Data,1),[1,1]))/2*10^-6;

    end
    % aSsign random length from sample distribution
    Vessel3{c}.Length = Length_data(randi(size(Length_data,1),[1,1]))*10^-6;
