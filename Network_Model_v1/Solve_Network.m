...
A = zeros(num_nodes,num_nodes);                              % Matrix to solve
...
%         iii(c) = i;
%         
%         c1{c} = {Node{i}.Parent_Node};
%         
%         cn1{c} = {Node{i}.Daughter_Node'};
%         %
        %
        %         col1 = [col1; Node{i}.Parent_Node'];
        %         rown1 = [rown1; i];
        %         coln1 = [coln1; Node{i}.Daughter_Node'];
%         c = c + 1;
    end
end
...
% end
% 
% for n = 1:size(Boundarynode,1)
%     %     if Node{n}.BC ~= 0 % if the node is a boundary value
%     %     A(n,:) = 0;
%     A(n,n) = 1;
%     b(n) = ((Node{n}.BC == 1) * BC_in) + ((Node{n}.BC == -1) * BC_out);
%     % Assumes presure infomation is given at the boundaries
% end
% %     end
% 
