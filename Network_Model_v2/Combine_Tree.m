...
function [Vessel,Node] = Combine_Tree(tree1,tree2)
%% Combine_Tree - Network_Model_v2
%{
    calls the different networks and combines them
%}
...
%     Vessel3{c}.xy_Length = 1;
    ...
    % No weights
    P_d = [5.973815080093145;1.164720946611277]; % gamma
    P_l = [1.843576621177406;1.826398550055475e+02]; % gamma

%     % Last half 5
%     P_d = [5.974298879626123;1.164625271272579]; % gamma
%     P_l = [2.344034363114109;1.483768794460348e+02]; % gamma
%     
%     % First half 5
%     P_d = [5.973815080093145;1.164720946611277]; % gamma
%     P_l = [1.661242926744869;2.082177034835431e+02]; % gamma
    
    
    dist_l = 'Gamma';

%     P_d = [8.960360999549838;3.093339797230252]; % weibull  
% %     P_l = [3.669278713298653e+02;1.408858454845507]; % weibull
    dist_d = 'Gamma';

    
    % No weights
%         P_d = [7.680710656531200;2.669384298538223]; % weibull  
%     P_l = [3.652953235452758e+02;1.426634881516861]; % weibull
%     dist_d = 'Weibull';
%        dist_l = 'Weibull';
%        
...
% % %         Vessel3{c}.Radius = Sample_Diameter("Capillary")/2*10^-6;
         Vessel3{c}.Radius = icdf(dist_d,rand(1),P_d(1),P_d(2)) + 2.2;
    Vessel3{c}.Radius = Vessel3{c}.Radius/2*10^-6;
%      Vessel3{c}.Radius = 25/2*10^-6;
    end
...
%     Vessel3{c}.Length = Sample_Length("Capillary")*10^-6;
        Vessel3{c}.Length = (icdf(dist_l,rand(1),P_l(1),P_l(2)) + 50)*10^-6;
%          Vessel3{c}.Length = (500)*10^-6;
