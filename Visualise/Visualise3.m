...
[Vessel2,Node2] = Solve_Network(Vessel,Node,n_P_0,n_P_inf,0);
[Vessel_Ref,Node_Ref] = Solve_Network(Vessel,Node,n_P_0,n_P_inf,1);
networkR_Ref = (d_P_0 - d_P_inf)/Vessel_Ref{1}.Flow;
networkstraightR = (d_P_0 - d_P_inf)/Vessel2{1}.Flow;

for v = 1:num_vessels
    radius(v) = Vessel{v}.Radius;
    res(v) = Vessel2{v}.Resistance_Poiseuille_effective;
    res_ref(v) = Vessel_Ref{v}.Resistance;
    flow(v) = Vessel2{v}.Flow;
    flow_ref(v) = Vessel_Ref{v}.Flow;
end

for n = 1:num_nodes
    pressure(n) = Node2{n}.n_Pressure;
...
networkvaricoseR = (d_P_0 - d_P_inf)/Vessel{1}.Flow;
for v = 1:num_vessels
    res1(v) = Vessel{v}.Resistance_Poiseuille_effective;
    flow1(v) = Vessel{v}.Flow;
%     Resistance_Poiseuille_effective
end
for n = 1:num_nodes
...
figure
histogram2(radius,res)
figure

histogram2(radius,res1)
...
for i = ["Arteriole","Venule"]
    ...
    rounded_res = min(abs(round(res*9)),9);
    for v = 1:num_vessels
        if Vessel{v}.Type == i
            if res(v) < 0
%                 plot([Vessel{v}.xy_Start(1),Vessel{v}.xy_End(1)],[Vessel{v}.xy_Start(2),Vessel{v}.xy_End(2)],'Color',[-res(v) 0 0 ] ,'LineWidth',linewidth(v))
%             draw_vessel_res = ['\draw[j',num2str(rounded_res(v)),',line width = ',num2str(linewidth(v),2),']', ...
             draw_vessel_res = ['\draw[db,line width = ',num2str(linewidth(v),2),']', ...
